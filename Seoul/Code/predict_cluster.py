...
def predict_volume(model, manager, ckpt, patient_path, model_outputs_dir, params_dict):
    ...
    volume_location = find_objects(np.sum(all_input_vols_orig, axis=-1) != 0)[0]
    ...
    if network_name == 'Unet':
        ...
        if predict_using_patches == True:
            predicted_vol_final = predict_patchwise(model, manager, ckpt, all_input_vols, params_dict)
        else:
...
def predict_patchwise(model, manager, ckpt, all_input_vols, params_dict):
    patch_size, batch_size, patch_overlap, boundary_removal, num_outputs, blend_windows_mode, number_snapshots_ensemble, percentage_empty_to_skip_patch, average_logits, logits_clip_value, predict_left_right_patch = unpack_keys(params_dict)
...
def predict_classification(model, manager, ckpt, all_input_vols, params_dict):
    ...
    for snapshot_number, saved_model_checkpoint in enumerate(reversed(manager.checkpoints)):
        if snapshot_number in number_snapshots_ensemble:
            if len(number_snapshots_ensemble) > 1 or snapshot_number != 0:
                ckpt.restore(saved_model_checkpoint).expect_partial()
            model_predictions[snapshot_number, :] = np.squeeze(model.predict(all_input_vols_batch, batch_size=batch_size))
    if average_logits == True:
